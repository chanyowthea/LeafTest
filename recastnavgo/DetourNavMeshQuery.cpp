...
__thread  dtNodePool* dtNavMeshQuery::m_tinyNodePool;
__thread  dtNodePool* dtNavMeshQuery::m_nodePool;
__thread  dtNodeQueue* dtNavMeshQuery::m_openList;
													 ...
													 dtPolyRef* randomRef, float* randomPt) const
{
	...
	if (PrepareNodePool() != DT_SUCCESS)
	{
		return DT_FAILURE | DT_OUT_OF_MEMORY;
	}

	if (PrepareOpenList() != DT_SUCCESS)
	{
		return DT_FAILURE | DT_OUT_OF_MEMORY;
	}
...
dtStatus dtNavMeshQuery::PrepareTinyNodePool() const
{
	if (!m_tinyNodePool)
	{
...
dtStatus dtNavMeshQuery::PrepareNodePool() const
{
	if (!m_nodePool)
	{
		m_nodePool = new (dtAlloc(sizeof(dtNodePool), DT_ALLOC_PERM)) dtNodePool(maxNodeCount, dtNextPow2(maxNodeCount / 4));
		if (!m_nodePool)
			return DT_FAILURE | DT_OUT_OF_MEMORY;
	}
	else
...
dtStatus dtNavMeshQuery::PrepareOpenList() const
{
	if (!m_openList)
	{
		m_openList = new (dtAlloc(sizeof(dtNodeQueue), DT_ALLOC_PERM)) dtNodeQueue(maxNodeCount);
										  ...
										  float* resultPos, dtPolyRef* visited, int* visitedCount, const int maxVisitedSize) const
{
	...
	if (PrepareTinyNodePool() != DT_SUCCESS)
	{
		return DT_SUCCESS | DT_OUT_OF_MEMORY;
	}
